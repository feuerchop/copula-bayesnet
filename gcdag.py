...
'''
Main routine for gaussian copula structure learning
...
class H3GCDiGraph(object):
   ...
   def fit(self, X):
      ...
      if self.method == 'spectral':
         ...
         '''
         if self.verbose:
            print colored('>> Running Riccati to estiamte precision matrix', color='blue')

         # TODO: note estimated cov is sample cov
         self.est_cov, self.precision_ = spectral(inv_norm_cdf, rho=2 * self.penalty, assume_centered=False)
         self.corr = scale_matrix(self.est_cov)
   ...
   def show(self, dir=None, output_pdf="untitled"):
      '''
      show network graph
      :return: the Graphviz object
      '''

      if self.conditional_independences_ is not None:
         dot = adj2dot(self.conditional_independences_, node_names=self.vertexes)
         dot.render(filename=output_pdf, directory=dir, view=True, cleanup=False)
         return dot
      else:
         print 'Network has no data to show, exit!'
         sys.exit(1)
...
if __name__ == '__main__':
   ...
   argparser = argparse.ArgumentParser(prog='gcdag',
                                       ...
                                       description='''
                                          paper code for ECML'17, where we try to use gaussian copula and L1-regularization
   ...
   argparser.add_argument("--output", help="Output path to save estiamted graph in Graphviz foramt.",
                          default="untitled")
   argparser.add_argument("--output_pdf", help="Output path to render Graphviz object to pdf file.",
   ...
   argparser.add_argument("--method", help="which method to run.. default ", default='ledoit_wolf')
   ...
   clf = H3GCDiGraph(alpha=args.alpha, method=args.method, vnames=node_names, pval=args.pval, bin=args.bins,
                     penalty=args.penalty)
   clf.fit(input_data)
   clf.show(output_pdf=output_pdf)
